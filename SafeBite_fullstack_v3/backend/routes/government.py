...
def government_login(payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    token = create_access_token(officer.id, officer.role, officer.state, officer.region)
    return {
        "access_token": token,
        "token_type": "bearer",
        "officer": {
...
def create_officer(payload: OfficerCreate, officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    if officer.role != "CENTRAL_ADMIN":
    ...
    return {"message": "Officer created", "officer_id": new_officer.officer_id}


def scoped_restaurants(officer: GovernmentOfficer, db: Session):
...
def dashboard(officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    restaurants = scoped_restaurants(officer, db)
    ...
    active = {"OPEN", "UNDER_INVESTIGATION", "ACTION_REQUIRED"}
    counts = {
        "establishments": len(restaurants),
        "devices": len(devices),
        "online_devices": sum(1 for d in devices if d.status == "ONLINE"),
        "critical_alerts": sum(1 for a in alerts if a.severity == "RED" and a.status in active),
        "high_risk_alerts": sum(1 for a in alerts if a.severity == "ORANGE" and a.status in active),
    ...
    return {
        "region": {"state": officer.state, "region": "ALL" if officer.role == "CENTRAL_ADMIN" else officer.region},
        "counts": counts,
...
def alert_details(alert_id: int, officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "sensor_evidence": [
            {"temperature": r.temperature, "humidity": r.humidity, "door_open": r.door_open, "timestamp": r.timestamp}
            for r in readings
        ],
        "investigation": {
            "id": investigation.id,
            ...
            "evidence": [
                {"id": e.id, "filename": e.original_filename, "sha256": e.sha256, "uploaded_at": e.uploaded_at}
...
def start_investigation(alert_id: int, officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    alert = db.query(Alert).filter(Alert.id == alert_id).first()
    if not alert:
        raise HTTPException(status_code=404, detail="Alert not found")
    enforce_region(officer, alert.restaurant)
    ...
    create_audit_record(
        ...
        investigation.id,
        officer.id,
        {"alert_id": alert.id, "restaurant_id": alert.restaurant_id},
    )
    db.commit()
...
def update_investigation(
    ...
    investigation.alert.status = "ACTION_REQUIRED" if status_value == "ACTION_REQUIRED" else "UNDER_INVESTIGATION"
...
def verify_investigation(
    ...
    db.commit()
    return {"message": "Investigation verified and case closed", "status": investigation.status, "audit_record_id": audit.id, "record_hash": audit.record_hash}


@router.get("/investigations")
def list_investigations(officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
        ...
        "investigations": [
            {
                ...
                "alert": {"id": i.alert.id, "severity": i.alert.severity, "risk_score": i.alert.risk_score, "reason": i.alert.reason, "status": i.alert.status},
                "restaurant": {"id": i.alert.restaurant.id, "name": i.alert.restaurant.name, "region": i.alert.restaurant.region},
            }
            for i in results
        ],
    }
...
def investigation_details(investigation_id: int, officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    investigation = db.query(Investigation).filter(Investigation.id == investigation_id).first()
    if not investigation:
        raise HTTPException(status_code=404, detail="Investigation not found")
    enforce_region(officer, investigation.alert.restaurant)
    return {
        ...
        "evidence": [{"id": e.id, "filename": e.original_filename, "sha256": e.sha256, "uploaded_at": e.uploaded_at} for e in investigation.evidence],
    }


@router.get("/investigations/{investigation_id}/audit")
def investigation_audit(investigation_id: int, officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    investigation = db.query(Investigation).filter(Investigation.id == investigation_id).first()
    if not investigation:
        raise HTTPException(status_code=404, detail="Investigation not found")
    enforce_region(officer, investigation.alert.restaurant)
    ...
    records = (
        ...
        .filter(BlockchainRecord.entity_type == "INVESTIGATION", BlockchainRecord.entity_id == investigation.id)
...
def establishments(officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    results = scoped_restaurants(officer, db)
    return {
        "total": len(results),
        "establishments": [
            {
                ...
                "status": r.status,
                "devices": [{"device_id": d.device_id, "status": d.status, "last_seen_at": d.last_seen_at} for d in r.devices],
            }
            for r in results
        ],
    }
...
@router.get("/devices")
def devices(officer: GovernmentOfficer = Depends(current_officer), db: Session = Depends(get_db)):
    results = []
    for restaurant in scoped_restaurants(officer, db):
        for device in restaurant.devices:
            results.append({
                "id": device.id,
